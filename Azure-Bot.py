# MTM2NDg1OTkwMTMyNjkyMTcyOA.GrrPSV.jB9ANDGC-QdfusGUMfBc2tACafw4sx8ml69kog
import discord
...
# Replace with your bot token and your Discord user ID
BOT_TOKEN = "MTM2NDg1OTkwMTMyNjkyMTcyOA.GrrPSV.jB9ANDGC-QdfusGUMfBc2tACafw4sx8ml69kog"
OWNER_ID = 703173684608630875  # Replace with your actual user ID
LISTINGS_FILE = "listings.json"  # safer to use JSON than plain .txt
...
async def view_listing(interaction: discord.Interaction, username: str):
    ...
    # Subtract the space used for title and formatting from the chunk size
    title_format = f"📄 **Listing for `{username}`**\n\n"
    max_length = 2000 - len(title_format)
    
    # Split the content into chunks of adjusted size
    chunks = [content[i:i+max_length] for i in range(0, len(content), max_length)]

    # Send each chunk as a separate message
    for i, chunk in enumerate(chunks):
        # Send the first message as a normal response, subsequent ones use followup to avoid interaction being considered finished.
        if i == 0:
...
async def calculate_tax(interaction: discord.Interaction, amount: float):
    tax_rate = 0.30  # Roblox's commission rate (30%)

    # Calculate before and after tax for the amount
    sell_price = amount / (1 - tax_rate)  # If you're selling for this amount to get 'amount' after tax
    receive_amount = amount * (1 - tax_rate)  # If you're receiving this amount after tax from selling
...
async def on_interaction(interaction: Interaction):
    if interaction.type == discord.InteractionType.component:
        if interaction.data['custom_id'] == "support_ticket":
            # Creating support ticket channel
            channel_name = f"support-{interaction.user.name}"
            # Change category to 'Important'
            category = get(interaction.guild.categories, name="Important")  # Updated to Important category
            overwrites = {
